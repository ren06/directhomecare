 ...
 * @property Mission $id
 */
class MissionLiveIn extends Mission {
...
//        }
    ...
    public static function createMissionLiveIn($bookingId, $missionPaymentId, $serviceLocationId, $serviceUserIds, $startDateTime, $endDateTime) {

        //copy address for mission
        $missionAddress = Address::copy($serviceLocationId, Address::TYPE_SERVICE_LOCATION_MISSION);

        $id = $missionAddress->id;

        //set all attributes
        $mission = new MissionLiveIn();

        $mission->id_address = $missionAddress->id;
        $mission->id_booking = $bookingId;
        $mission->id_mission_payment = $missionPaymentId;
        $mission->type = self::TYPE_LIVE_IN;
        ...
        //ServiceUser::assignToMission($serviceUserIds, $bookingId);

        ServiceUser::copyAndAssignToMission($serviceUserIds, $mission->id);
    ...
    public function getEntityPrice($user) {
        ...
        return Prices::getPrice($user, Prices::LIVE_IN_DAILY_PRICE, $creationDate);
    }
